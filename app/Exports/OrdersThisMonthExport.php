...
class OrdersThisMonthExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents, WithColumnFormatting, WithMapping
{
    /**
    ...
    */
    ...
    {
        $orders = Order::where('created_at', '>=', \Carbon\Carbon::now()->startOfMonth())->get();
        foreach($orders as $or) {
            if(!$or->user_name && !$or->user_surname) {
                $or->user_name = $or->user->name;
                $or->user_surname = $or->user->surname;
                $or->user_phone = $or->user->phone;
            }
